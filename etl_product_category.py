...
def extract():
    ...
    try:
        ...
        df.sort_values(by='ProductCategoryKey', inplace=True)

        # Memuat data ke PostgreSQL
        load(df, 'DimProductCategory')
...
def transformation(df):
    ...
    df['ProductCategoryAlternateKey'] = df['ProductCategoryKey']
    ...
    col_order.insert(col_order.index('ProductCategoryKey') + 1, col_order.pop(col_order.index('ProductCategoryAlternateKey')))
    df = df[col_order]
